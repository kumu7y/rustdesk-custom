...
def main() -> int:
    ...
    if entries:
        server_section = (
            ...
            # Each entry already ends with its own trailing comma, so entries
            # are joined with bare newlines only.
            + "\n".join(entries) + "\n];")
    else:
        server_section = (
...
'''
    ...
    with open(out_path, "w", encoding="utf-8", newline="\n") as fh:
        fh.write(module)
    # Regression guards: a syntax error anywhere in the module makes rustc
    # drop EVERY symbol inside it, which surfaces as unrelated E0425 errors
    # at the call sites. Catch malformed literals here instead.
